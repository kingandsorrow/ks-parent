...
public class ToolUtil {
     ...
     * @since JDK 1.6
     */
    public static final String getDateToStr(Date date) {
        if (date == null) {
            return null;
        }
        DateFormat df = new SimpleDateFormat("yyyy/MM/dd HH:mm:ss");
    ...
    /**
     * d1 / d2 ,保留scale 位小数 四舍五入 function:
     *
     * @param d1
     * @param d2
     * @param scale
     ...
     * @since JDK 1.6
     */
    public static double divideRounding(double d1, double d2, int scale) {
     ...
     * @Title encryptPhone
     * @Description 加密手机号中间四位
     * @author rookie
     */
    public static String encryptPhone(String oldPhone) {
        if (ToolUtil.isEmpty(oldPhone) || oldPhone.length() < 11) {
            return oldPhone;
        }

        String newPhone = oldPhone.replaceAll("(\\d{3})\\d{4}(\\d{4})", "$1****$2");
    ...
    public static void main(String[] args) {
        //System.out.println(ToolUtil.divideRounding(8, 3, 2));
//		Calendar cal = Calendar.getInstance();
...
//        System.out.println(boolVal("0"));
        ...
        System.out.println(appendPhone(""));
    }
    ...
    public static String getStringID() {
        return SequenceHelper.getNextSequence();
    }
     ...
     * @throws
     * @Title: readProperty
     * @Description: TODO(根据key读取配置文件中的一个字符串)
     */
    public static String readProperty(String key) {
        ...
        if (proper == null) {
            try {
                ...
                proper.load(new InputStreamReader(ToolUtil.class
                        .getResourceAsStream("/message.properties")));
            } catch (IOException e) {
                logger.error(LogFormat.formatError("ToolUtil.readProperty", e,
     ...
     * @return java.lang.String
     * @Title getHostByUrl
     * @Description 根据url字符串地址获取域名
     * @author rookie
     */
    public static String getHostByUrl(String urlLinkStr) {
        String host = null;

        if (ToolUtil.isEmpty(urlLinkStr)) {
            return host;
        }

        try {
            URL url = new URL(urlLinkStr);
            host = url.getHost();
        } catch (MalformedURLException e) {
            logger.info(LogFormat.formatMsg("ToolUtil.getHostByUrl", "get host by url happen exception:", urlLinkStr));
            e.printStackTrace();
        }
