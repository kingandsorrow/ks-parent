...
import top.ks.common.redis.KeyPrefix;
...
public class JedisUtil {
     ...
     * @return
     */
    public static Object getObjectVal(KeyPrefix keyPrefix, String key) {
        Object obj = null;
        ShardedJedis client = getInstance();
        try {
            byte[] bytes = client.get((keyPrefix.getPrefix() + key).getBytes());
            if (bytes != null && bytes.length > 0) {
                obj = unserialize(bytes);
            }
        } catch (Exception e) {
     ...
     * @param seconds 存活时间,单位/秒
     */
    public static String setObjectVal(KeyPrefix keyPrefix, String key, Object obj) {
        String result = null;
        ShardedJedis client = getInstance();
        try {
            if (keyPrefix.expireSeconds() > 0) {
                result = client.setex((keyPrefix.getPrefix() + key).getBytes(), keyPrefix.expireSeconds(), serialize(obj));
            } else {
                result = client.set((keyPrefix.getPrefix() + key).getBytes(), serialize(obj));
            }
        } catch (Exception e) {
            logger.error(e.getMessage(), e);
        } finally {
     ...
     * @return
     */
    public static Object setObjectVal(KeyPrefix keyPrefix, String key) {
        Object obj = null;
        ShardedJedis client = getInstance();
        try {
            byte[] bytes = client.get((keyPrefix.getPrefix() + key).getBytes());
    ...
    public static long repeatCheck(CommonKey repeatCheck, String repeatCheckKey) {
